...
export default function Home() {
  ...
  const downloadPDF = () => {
    ...
    const reportContent = `
Mamacare Pregnancy Report
========================

Generated on: ${new Date().toLocaleDateString()}

Pregnancy Week: ${profile.pregnancyWeek}
Due Date: ${dueDate || 'Not calculated'}

Symptom History:
${profile.symptoms.map(symptom => 
  `- ${symptom.symptom} (${symptom.severity}): ${symptom.notes} (${new Date(symptom.loggedAt).toLocaleDateString()})`
).join('\n') || 'No symptoms logged'}

Checkup Frequency: Every ${profile.reminderFrequencyWeeks} week(s)
Custom Notes: ${profile.customNotes || 'None'}

Reflections:
${profile.reflections?.map(reflection => 
  `- ${reflection.text} (${new Date(reflection.timestamp).toLocaleDateString()})`
).join('\n') || 'No reflections recorded'}

This report was generated by Mamacare to help you share your pregnancy journey with your care team.
    `;
    ...
    a.href = url;
    a.download = `mamacare-report-${new Date().toISOString().slice(0, 10)}.txt`;
  ...
  const fetchRegisteredUsers = async () => {
    ...
    try {
      // In a real implementation, you would query the Firestore collection
      // For now, we'll simulate with mock data
      const mockUsers = [
        { id: 1, name: "Sarah Johnson", joinDate: "2025-10-15" },
        { id: 2, name: "Emily Chen", joinDate: "2025-10-22" },
        { id: 3, name: "Maria Rodriguez", joinDate: "2025-11-01" },
        { id: 4, name: "Priya Patel", joinDate: "2025-11-05" },
        { id: 5, name: "Amanda Williams", joinDate: "2025-11-10" },
        { id: 6, name: "Jessica Kim", joinDate: "2025-11-12" },
        { id: 7, name: "Olivia Brown", joinDate: "2025-11-18" },
        { id: 8, name: "Sophie Davis", joinDate: "2025-11-20" },
      ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#fffaf5] via-[#fff1ea] to-[#ffeae0] pb-24">
      ...
      <main id="dashboard" className="mx-auto mt-12 grid max-w-6xl gap-6 px-4 sm:px-6 lg:px-8">
        ...
        <section className="grid gap-6 lg:grid-cols-3">
          <Card className="lg:col-span-2 cursor-pointer hover:bg-secondary/10 transition-colors" onClick={fetchRegisteredUsers}>
            <CardHeader className="flex flex-row items-start justify-between">
              <div>
            ...
            <CardContent className="space-y-4">
              {showRegisteredUsers ? (
                <div className="space-y-3">
                  <h3 className="text-lg font-semibold">Registered Users ({registeredUsers.length})</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-60 overflow-y-auto">
                    {registeredUsers.map((user) => (
                      <div key={user.id} className="flex items-center justify-between p-3 rounded-lg border bg-secondary/5">
                        <span className="font-medium">{user.name}</span>
                        <span className="text-xs text-muted-foreground">Joined {new Date(user.joinDate).toLocaleDateString()}</span>
                      </div>
                    ))}
                  </div>
                  <Button 
